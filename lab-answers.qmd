...
⬆️ Is just a plain image.

```{r}
# Now we are using an additional package, plotly, to make an interactive plot.
library(plotly)
...
⬆️ This is an interactive graph.
...
ggplot(
  data = class_grades, mapping = aes(x = lab_group)
) +
  geom_bar()
```
...
⬆️ There are some differences, but this isn't that different from Excel.
...
- Helpful Hints:
  ...
  - Find the `mean` of the `lab_grade` column.
